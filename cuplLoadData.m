function analysis=cuplLoadData(analysis)
...
nCells = length(an.dataFilenames);
sisterCount = zeros(nCells,1); % Nx1 (N: # of cells)
trackCount = zeros(nCells,1); % Nx1 (N: # of cells)
...
an.nLoadedCells = nCells;
an.maxTrackLength = size(an.sisterCoords1,1);
% Time vector.
an.time = (0:an.maxTrackLength-1)'*an.options.frameInterval;
